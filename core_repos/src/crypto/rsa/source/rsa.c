...
/* RSAES-OAEP private-key decryption, according to PKCS #1.
 */
int RSAOAEPPrivateDecrypt (output, input, privateKey)
unsigned char *output;                                      /* output block */
unsigned char *input;                                        /* input block */
R_RSA_PRIVATE_KEY *privateKey;                           /* RSA private key */
{
  ...
  if (status = RSAPrivateBlock
      (pkcsBlock, &pkcsBlockLen, input, modulusLen, privateKey))
    return (status);
  ...
  R_memcpy ((POINTER)output, (POINTER)&pkcsBlock[0], modulusLen);
