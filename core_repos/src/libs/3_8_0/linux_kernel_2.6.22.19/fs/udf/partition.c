...
_read_mirror:
		/*
			The metadata mirror file was not accessed yet and there was an error reading from the metadata file
			hence we switch to the mirror file.
		*/
		if (UDF_SB_TYPEMETA(sb, partition).s_mirror_fe == NULL)
		{
			/* check the mirror file entry */
			addr.logicalBlockNum = UDF_SB_TYPEMETA(sb, partition).s_mirror_file_loc;
			addr.partitionReferenceNum = UDF_SB_PARTNUM(sb, partition);

			udf_debug("MIRROR block=%d part=%d\n",addr.logicalBlockNum,addr.partitionReferenceNum);

			UDF_SB_TYPEMETA(sb, partition).s_mirror_fe = udf_iget(sb, addr);
