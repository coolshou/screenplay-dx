...
typedef enum {
     ...
     SBF_FOREIGN_VIDEO   = 0x00000008,  /* video memory is preallocated by
                                           application, won't be freed */
} SurfaceBufferFlags;
...
struct _SurfaceBuffer
{
     ...
     struct {
          ...
          int                offset;    /* byte offset from the beginning
                                           of the framebuffer */
          u32                dram;      /* DRAM address for FOREIGN_VIDEO case. <b>Sigma extension</b>*/
          void              *addr;      /* mapped address for FOREIGN_VIDEO case. <b>Sigma extension</b>*/
...
typedef void (*wait_software_access_callback)(CoreSurface *surface);
...
struct _CoreSurface
{
	 ...
	 wait_software_access_callback soft_callback;

     DFBStereoscopicFormat  prev_flip_stereo_fmt; /* stereo format of the previous Flip (DSSF_NONE for 2D) */
     int                    prev_flip_stereo_off; /* stereo offset of the previous Flip */
};
...
void dfb_surface_set_wait_software_access_callback( CoreSurface *surface, wait_software_access_callback callback );
