	...
	*(pDest + 2) = 0; 		/* Set length to 0 first.  Need to update to real length in the end of this function. */
	...
	for (i = 0;i < MAX_P2P_GROUP_SIZE;i++)
	{
		...
		if (pClient->P2pClientState == P2PSTATE_CLIENT_WPS_DONE)
		{
			...
			DBGPRINT(RT_DEBUG_TRACE, ("  -- InsertP2PGroupInfoTlv( ) (Mac  %02x:%02x:%02x:%02x:%02x:%02x.\n", PRINT_MAC(pClient->addr)));
...
/*	MLME_P2P_ACTION_STRUCT	P2PActReq; */	
	...
	DBGPRINT(RT_DEBUG_ERROR, ("P2P - P2pReceDevDisReqAction = %ld ", Elem->MsgLen));
	...
	pMacEntry = MacTableLookup(pAd, devAddr);
	DBGPRINT(RT_DEBUG_ERROR, ("P2P - ask for p2p client[%d] = %02x:%02x:%02x:%02x:%02x:%02x.\n", p2pClientIndex, PRINT_MAC(devAddr)));

	if ((pMacEntry != NULL) &&(p2pClientIndex < MAX_P2P_GROUP_SIZE) 
		/*&& (IS_P2P_PEER_CLIENT_OP(&pAd->P2pTable.Client[p2pClientIndex]))) */
		&& (pAd->P2pTable.Client[p2pClientIndex].P2pClientState == P2PSTATE_CLIENT_WPS_DONE))
	{
		MLME_P2P_ACTION_STRUCT	P2PActReq;	
...
/*		P2PSendDevDisRsp(pAd, P2PSTATUS_SUCCESS, pFrame->Token, pFrame->p80211Header.Addr2, &TotalFrameLen);*/
	...
	IN LARGE_INTEGER   TimeStamp) 
{
	...
	{
		DBGPRINT(RT_DEBUG_ERROR, ("1 PeerP2pBeaconProbeRspAtScan failed :  wcid = %d. not ValidAsP2P. Bug!please check. \n", Elem->Wcid));
		DBGPRINT(RT_DEBUG_ERROR, ("pMacEntry Addr = %02x:%02x:%02x:%02x:%02x:%02x.\n", PRINT_MAC(pMacEntry->Addr)));
		return;
	}
